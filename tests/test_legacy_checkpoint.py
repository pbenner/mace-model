...
def test_legacy_checkpoint_imports_are_scoped(tmp_path):
    ...
    with legacy_checkpoint_imports():
        ...
        extensions_mod = importlib.import_module('mace.modules.extensions')
        ...
        assert hasattr(extensions_mod, 'PolarMACE')
...
def test_resolve_foundation_checkpoint_accepts_existing_local_path(tmp_path):
    ...
    for source in ('mp', 'off', 'omol', 'anicc', 'polar'):
...
def test_get_mace_polar_names_exposes_named_variants():
    names = get_mace_polar_names()
    assert names == ['polar-1-s', 'polar-1-m', 'polar-1-l']
